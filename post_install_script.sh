...
sudo zypper -v -y dup --from packman --allow-vendor-change

sudo zypper -v -y install opi
opi codecs

echo "Removing unnecessary packages and installing extra ones..."
sudo zypper -v -y rm --clean-deps discover kmail kontact kmines akregator kaddressbook korganizer kompare konversation tigervnc kleopatra kmahjongg kpat kreversi ksudoku
sudo zypper -v -y in fish neofetch htop kwrite

echo "Installing build tools..."
sudo zypper -v -y in -t pattern devel_basis

echo "Installing microsoft fonts..."
sudo zypper -v -y in fetchmsttfonts
